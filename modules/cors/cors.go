...
func New(options Options) *Cors {
	...
	if len(options.AllowedMethods) == 0 {
		c.allowedMethods = []string{"GET", "POST", "PUT", "DELETE", "CONNECT", "HEAD"}
	} else {
...
func (c *Cors) handlePreflight(ctx *eject.Context) bool {
	...
	ctx.Response.SetHeader("Vary", "Access-Control-Request-Method")
	ctx.Response.SetHeader("Vary", "Access-Control-Request-Headers")
	...
	ctx.Response.SetHeader("Access-Control-Allow-Methods", strings.ToUpper(reqMethod))
	if len(reqHeaders) > 0 {

		// Spec says: Since the list of headers can be unbounded, simply returning supported headers
		// from Access-Control-Request-Headers can be enough
		ctx.Response.SetHeader("Access-Control-Allow-Headers", strings.Join(reqHeaders, ", "))
	}
	if c.allowCredentials {
		ctx.Response.SetHeader("Access-Control-Allow-Credentials", "true")
	}
	if c.maxAge > 0 {
		ctx.Response.SetHeader("Access-Control-Max-Age", strconv.Itoa(c.maxAge))
	}
...
func (c *Cors) handleActualRequest(ctx *eject.Context) {
	...
	if len(c.exposedHeaders) > 0 {
		ctx.Response.SetHeader("Access-Control-Expose-Headers", strings.Join(c.exposedHeaders, ", "))
	}
	if c.allowCredentials {
		ctx.Response.SetHeader("Access-Control-Allow-Credentials", "true")
	}
